...
local browser = {
  ...
  jump_hint_mode = function(input)
    local value = input("nvim-browser hint: ")
    table.insert(calls, "jump_hint:" .. value)
  end,
  ...
  yank_point_url_here = function(register)
    table.insert(calls, "yank_point_url:" .. tostring(register))
    return true
  end,
  ...
  activate_here = function()
    table.insert(calls, "activate_here")
  end,
...
keymaps.setup_buffer(browser, first_bufnr, {
  ...
  input = function(prompt)
    ...
    elseif prompt == "nvim-browser select at cursor: " then
...
assert_buffer_mapping(first_bufnr, "gj", "buffer-local controls should install hinted cursor jump mapping")
...
assert_buffer_mapping(first_bufnr, "ga", "buffer-local controls should install smart cursor activation")
...
assert_no_buffer_mapping(first_bufnr, "ga", "false buffer-local smart activation mapping should disable default")
...
assert_buffer_mapping(first_bufnr, "gL", "custom address prompt shortcut should be installed")
...
keymaps.setup_buffer(browser, canceled_type_bufnr, {
  ...
  input = function(prompt)
    ...
    if prompt == "nvim-browser select at cursor: " then
      return ""
    end
    return "unexpected"
  end,
...
trigger_buffer(canceled_type_bufnr, "gO")
assert(#calls == calls_before_canceled_type, "empty or canceled cursor prompts should not call browser cursor actions")
