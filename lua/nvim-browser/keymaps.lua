...
function M.setup_buffer(browser, bufnr, opts)
  ...
  set_mapping(nil, mapping_lhs(mappings, "follow_point_url_here", "gf"), function()
    if browser.follow_point_url_here ~= nil then
      browser.follow_point_url_here()
    end
  ...
  set_mapping(nil, mapping_lhs(mappings, "type_here", "gI"), function()
    if browser.type_here == nil then
      return
    end
    local text = input("nvim-browser type at cursor: ")
    if text == nil or text == "" then
      return
    end
    browser.type_here(text)
  end, "nvim-browser: type at cursor", buffer_opts)

  set_mapping(nil, mapping_lhs(mappings, "submit_here", "gS"), function()
    if browser.type_here == nil then
      return
    end
    local text = input("nvim-browser submit at cursor: ")
    if text == nil or text == "" then
      return
    end
  ...
  set_mapping(nil, mapping_lhs(mappings, "yank_point_url_here", "gY"), function()
    if browser.yank_point_url_here ~= nil then
      browser.yank_point_url_here(vim.v.register)
    end
  ...
  end, "nvim-browser: right click preview", buffer_opts)
